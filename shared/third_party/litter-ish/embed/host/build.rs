...
fn main() {
    ...
    // Cargo cannot infer Meson's inputs. Without these declarations a change
    // to security-sensitive C code (for example fs/real.c confinement) can be
    // silently omitted from the final Rust static library until an unrelated
    // clean build happens.
    for source_dir in ["asbestos", "emu", "fs", "kernel", "platform", "util"] {
        emit_rerun_if_changed_recursive(&ish_root.join(source_dir));
    }
    for source_file in ["debug.h", "main.c", "meson.build", "meson_options.txt", "misc.h"] {
        println!("cargo:rerun-if-changed={}", ish_root.join(source_file).display());
    }
