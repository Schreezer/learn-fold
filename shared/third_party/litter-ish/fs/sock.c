...
int_t sys_socket(dword_t domain, dword_t type, dword_t protocol) {
    ...
    // Learnfold's course shell irreversibly drops to the course owner's
    // non-root uid before executing agent-authored scripts. Keep local Unix
    // IPC available, but do not let that confined shell turn BusyBox (or an
    // uploaded executable) into an undeclared network client. The ordinary
    // root-owned iSH runtime retains its existing networking behavior.
    if (!superuser() && (domain == AF_INET_ || domain == AF_INET6_))
        return _EPERM;
