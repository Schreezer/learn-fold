...
dword_t sys_mount(addr_t source_addr, addr_t point_addr, addr_t type_addr, dword_t flags, addr_t data_addr) {
    // Linux requires CAP_SYS_ADMIN for mount(2). iSH does not model
    // capabilities, so preserve the security boundary with its existing
    // superuser check. Without this, a dropped-uid course process can mount
    // any host path reachable by realfs and escape its chroot.
    if (!superuser())
        return _EPERM;
