...
enum AgentAssistedPairing {
    ...
    `npx -y learnfold-link@0.3.6 handoff "\(submitURL.absoluteString)"`
    ...
    1. Run `npx -y learnfold-link@0.3.6 rotate` to invalidate every existing Learnfold pairing.
    2. Run `npx -y learnfold-link@0.3.6 uninstall` to disable autostart and stop the managed service.
    3. If a Learnfold Link or Alleycat daemon is still running, run \
    `npx -y learnfold-link@0.3.6 stop`.
