...
APP_BUNDLE_ID="${APP_BUNDLE_ID:-com.chirag.learnfold}"
...
LIVE_ACTIVITY_BUNDLE_ID="${LIVE_ACTIVITY_BUNDLE_ID:-com.chirag.learnfold.liveactivity}"
LIVE_ACTIVITY_PROVISIONING_PROFILE_SPECIFIER="${LIVE_ACTIVITY_PROVISIONING_PROFILE_SPECIFIER:-}"
WATCH_BUNDLE_ID="${WATCH_BUNDLE_ID:-com.chirag.learnfold.watchkitapp}"
WATCH_PROVISIONING_PROFILE_SPECIFIER="${WATCH_PROVISIONING_PROFILE_SPECIFIER:-}"
WATCH_COMP_BUNDLE_ID="${WATCH_COMP_BUNDLE_ID:-com.chirag.learnfold.watchkitapp.complications}"
...
if [[ "$TESTFLIGHT_SKIP_BUILD" != "1" ]]; then
    ...
    archive_cmd=(
        ...
        # TestFlight uses per-installation guest access. Never embed a shared
        # token from the developer's environment or project configuration.
